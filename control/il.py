...
# ==========================================
try:
    # 保持 U-Net 感知层路径，使用优化后的 OpenCV 接口
        ...
        try:
            if key.char == '1': key_states['forward'] = True
                ...
                if not key_states['autopilot_toggle_pressed']:
                    ...
                    state = "开启 (V3 混合控制: ConvLSTM + 几何限速)" if key_states['autopilot_on'] else "关闭 (手动)"
                    print(f"\n>>> 自动导航模式已 {state}")
...
print("\n" + "="*60)
print("     结肠镜仿真系统 (V3: ResNet18 + ConvLSTM)")
print("="*60 + "\n")
        ...
        while viewer.is_running():
            ...
            if key_states['autopilot_on']:
                if center_coords is not None:
                    ...
                    # 此处调用的是 v3.api_convlstm3f 中的 predict
