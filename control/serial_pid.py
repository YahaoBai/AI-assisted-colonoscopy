...
try:
    if not os.path.exists('../xml/colon_scene.xml'):
        ...
        model = mujoco.MjModel.from_xml_path('../xml/colon_scene.xml')
...
print("\n>>> 正在初始化底层通信链路...")
try:
    pid_serial = serial.Serial('/dev/pts/3', baudrate=115200, timeout=0)
    print("✅ 虚拟串口 /dev/pts/3 (TX) 已成功挂载！")
except Exception as e:
    print(f"❌ 虚拟串口挂载失败: {e}")
    pid_serial = None
...
try:
    with mujoco.viewer.launch_passive(model, data) as viewer:
        ...
        while viewer.is_running():
            ...
            print(f"理论物理步长: {dt} 秒，理论帧率: {1.0 / dt} Hz")
            ...
            if key_states['autopilot_on']:
                if center_coords is not None:
                    ...
                    # [新增] 串口底层数据下发
                    # --------------------------------------------------
                    if pid_serial is not None:
                        tx_frame = f"{step_yaw:.4f},{step_pitch:.4f}\n"
                        pid_serial.write(tx_frame.encode('utf-8'))
